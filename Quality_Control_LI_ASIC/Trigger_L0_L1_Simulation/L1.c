...
#define gain 0.870/0.750//0.870/0.965//0.870*1.324//1.160//0.914 //True gain: 0.876, effect from the bandwidth.

int L1(float THRESHOLD, int Nevents, int printoption)
{
  ...
  int rt[7];//retardo temporal en las señales de entrada
  double rgain[7];//variacion en la ganancia de las amplitudes de entrada
  double sigma;
  ...
  double signal_in[7][200];
  double offset[3] = {-18.3-6.138,-18.3-6.138,-18.3-6.138};//{-3.45-6.138, -3.45-6.138, -3.45-6.138}; //{-6.138-gain*21,-6.138-gain*21,-6.138-gain*21}; //offset que meten los sumadores.Para clipping 60,offset+gain*21
  ...
  FILE *fp[Nevents],*fin[Nevents], *fadd[Nevents], *fcomp[Nevents], *fphe[Nevents], *fswitch; // Archivos que genera el programa: las señales de entrada, las salidas de los sumadores y la saluda del comparador. 
  ...
  for (event = 0; event < Nevents; event++){
    ...
    //====================SEÑAL DE ENTRADA PARA L1 PROVIENE DE L0========================//

   

    sprintf(fname, "adder[%d].txt", event);
    fp[event] = fopen(fname, "r");
    
    if (fp==NULL)
      fprintf(stderr, "Couldn't open file adder[%d].txt",event);

    i=0;
    while (!feof(fp[event]))
      {
	fscanf(fp[event],"%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf ", &t, &signal_and_noise[0][i],&signal_and_noise[1][i],&signal_and_noise[2][i],&signal_and_noise[3][i],&signal_and_noise[4][i],&signal_and_noise[5][i],&signal_and_noise[6][i]);
	i++;
      }
    ...
    //==========Aquií incluyo  la variacion en la ganancia y el retardo temporal de la señal por canales. ===================//
    for (i=0; i<7; i++){
    srand((i+1));
    rgain[i]= ((rand()%200000)-100000)/10000.0;
      for(j=0; j<200; j++){

	signal_and_noise[i][j]=signal_and_noise[i][j]+rgain[i]*(-1.0);
      }
    }
	...
	if (j+rt[i]<=0 || j+rt[i] > 200){
	  signal_in[i][j]=0;	
	  }
	else
	  signal_in[i][j] =  signal_and_noise[i][j+rt[i]];
      }
    }

    if (printoption==1){

    sprintf(fname, "inputL1[%d].txt", event);
    fin[event] = fopen(fname, "w");
    
    if (fin==NULL)
      fprintf(stderr, "Couldn't open file inputL1[%d].txt",event);


    t=-10; 
    for (j=0; j< 200; j++){
      t=t+0.1;
      fprintf(fin[event],"%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",t, signal_in[0][j], signal_in[1][j], signal_in[2][j], signal_in[3][j], signal_in[4][j], signal_in[5][j], signal_in[6][j]);
    }
 ...
 fclose(fin[event]);
  ...
  for (i=0; i < 7; i++){
	...
	if (switch_status[i][j]== 0) // Si el interruptor está apagado, la señal de salida replicada es 0.
	  ...
	  replic_signal[i][j][k]=signal_in[i][k];
  ...
  for (k=0; k<200; k++){
	...
	sum[j][k]+=gain*replic_signal[i][j][k];
      }
    }
  }
  ...
  for (j=0; j< 3; j++){
	...
	if (printoption==1){
	  ...
	  sprintf(fname, "L1adder[%d].txt", event);
	  ...
	  for (k=0;k<200; k++){
	    t=t+0.1;
	    fprintf(fadd[event],"%f\t%f\t%f\t%f\t%f\n", t, sum[0][k],sum[1][k],sum[2][k],THRESHOLD);
	  }
