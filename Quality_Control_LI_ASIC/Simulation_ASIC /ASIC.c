...
#define THRESHOLD 10.0  //Threshold elegido en mVoltios
#define B 50.0 //
...
int main (void)
{
  ...
  double snoise[7]; // Sigma del ruido
  double noise[7][200]; 
  ...
  fnoise = fopen("lowth_noise.txt","r");
  
  if(fnoise==NULL)
    fprintf(stderr,"Couldn't open file noise.txt\n");

  i = 0;
  while (!feof(fnoise))
    {
      fscanf(fnoise,"%lf ", &snoise[i]);
      i++;
    }
  fclose(fnoise);
  ...
  */
  ...
  for (j=0; j<7; j++){
    ...
    for (ti=-10.0; ti<10.0; ti= ti + 0.1){

      for (tj=-10.0; tj<10.0; tj=tj+0.1){

	if (ti==tj)
	  
	  input_signal[j][i] += PHE[j]* PHE_CONV * (1.0/(sigma*sqrt(2.0*PI))*exp(-pow(tj,2)/(2.0*(pow(sigma, 2)))))*1/(2*PI*B);
	
	
	else{
	input_signal[j][i] += PHE[j]* PHE_CONV * (1.0/(sigma*sqrt(2.0*PI))*exp(-pow(tj,2)/(2.0*(pow(sigma, 2)))))*sin((ti-tj)*2*PI*B)/(2*PI*B);
	}     
      }
      i++;
    }
