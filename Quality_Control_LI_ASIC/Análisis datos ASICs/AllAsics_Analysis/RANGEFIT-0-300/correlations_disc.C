  ...
  for (disc = 0; disc < 2; disc++){
	...
	for (sum = 0; sum < 1 ; sum++){
	  ...
	  if (discy != disc){
	    ...
	    gr[disc][channel][sum]->GetXaxis()->SetTitle(Form("Discriminator %i[mV]",disc));
	    gr[disc][channel][sum]->GetYaxis()->SetTitle(Form("Discriminator %i[mV]",discy));
	    ...
	    gr2[disc][channel][sum]->GetXaxis()->SetTitle(Form("Discriminator %i[mV]",disc));
	    gr2[disc][channel][sum]->GetYaxis()->SetTitle(Form("Discriminator %i[mV]",discy));
