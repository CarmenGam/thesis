void graficallanalog(){
  ...
  ofstream peaks("peaks.txt");

  float outlier;
  int asic, disc, channel, sum, count1=0, count2=0, count3=0, count4=0;
  ...
  for (int irow=0; irow < lines ; irow++){
    ...
    if ((analog > -3 && analog < -1.9) || (analog > 8 && analog < 9.9) || (analog > -14 && analog < -12.9) || (analog > -25 && analog < -23.3) ){

      
      peaks << asic << "\t" << disc << "\t" << channel << "\t" << sum << "\t" << analog  << endl;
