...
void gaincorrelation(int printoption)

{
  ...
  float *row_content;
  ofstream fcorr("gain_channel_coeff.txt");
  ...
  for (disc = 0; disc < 2; disc++){
	...
	if (channely != channel){
	  ...
	  for ( sum = 0; sum < 3 ; sum++){
	    ...
	    gr[disc][channel][sum]->GetXaxis()->SetTitle(Form("Gain of Channel %i",channel));
	    gr[disc][channel][sum]->GetYaxis()->SetTitle(Form("Gain of Channel %i",channely));
	...
	fcorr << disc << "\t" << sum <<  "\t" << channel << "\t" << channely <<  "\t" << corr << "\t" << chi2 << "\t" << ndf << endl;
	
	
	
	if (printoption==0){
	  canvas->SaveAs(Form("GAINCORR-CH%iCH%iDISC%iSUM%i.gif",channel,channely,disc,sum));
	}
	if (printoption==1){
	  canvas->SaveAs(Form("GAINCORR-CH%iCH%iDISC%iSUM%i.pdf",channel,channely,disc,sum)); 
	}	
	  }
	}
      }
