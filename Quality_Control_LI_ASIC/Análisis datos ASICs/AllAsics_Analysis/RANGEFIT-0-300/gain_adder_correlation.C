...
void gain_addercorrelation(int printoption)

{
  ...
  ofstream fcorr("gain_adders_coeff.txt");
  ...
  for (disc = 0; disc < 2; disc++){
	...
	for (sumy = 0; sumy < 3; sumy++){
	  ...
	  if (sumy != sum){
	    ...
	    TString t= Form("Gain Correlation Adder %c - %c Disc %i Channel %i",'A' + sum,'A' + sumy, disc,channel);
	    TCanvas *canvas = new TCanvas(t,t);
	    
	    for(int i=0; i< Nasics ; i++){
	      
	      x[i] = gain[i][disc][channel][sum];
	      y[i] = gain[i][disc][channel][sumy];
	    ...
	    gr[disc][channel][sum]->GetXaxis()->SetTitle(Form("Gain of Adder %c",'A' +sum));
	    gr[disc][channel][sum]->GetYaxis()->SetTitle(Form("Gain of Adder %c",'A' +sumy));
	    ...
	    fcorr << disc << "\t" << channel <<  "\t" << sum << "\t" << sumy <<  "\t" << corr << "\t" << chi2 << "\t" << ndf << endl;


	    if (printoption==0){
	      canvas->SaveAs(Form("GAIN_ADDCORR-CH%iDISC%iSUM%cSUM%c.gif",channel,disc,'A'+sum,'A'+sumy));
	    }
	    if (printoption==1){
	      canvas->SaveAs(Form("GAIN_ADDCORR-CH%iDISC%iSUM%cSUM%c.gif",channel,disc,'A'+sum,'A'+sumy)); 
	    }	    	    
