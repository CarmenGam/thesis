...
void offcorrelation(int printoption)

{
 ...
 TFile in_file("allasicsdata.root");
  ...
  ofstream fcorr("fit_coefficients.txt");
 ...
 if ((row_content[9] < 0.7) || (row_content[9] > 1.0)){
  ...
  for (disc = 0; disc < 2; disc++){
    for (channel = 1; channel <= 2 ; channel++){
      ...
      for (channely = 1; channely <= 2; channely++){
	
	if (channely != channel){
	  
	  for ( sum = 0; sum < 1 ; sum++){
	    
	    TString t= Form("Channel %i - %i Disc %i Adder %c",channel,channely, disc,'A'+sum);
	    TString t1= Form("Channel %i - %i Disc %i Adder %c",channel,channely, disc,'A'+sum);
	    TString t2= Form("Channel %i - %i Disc %i Adder %c",channel,channely, disc,'A'+sum);
	    ...
	    fcorr << disc << "\t" << sum <<  "\t" << channel << "\t" << channely <<  "\t" << corrdiff << "\t" << corr << "\t" << chi2diff << "\t" << ndfdiff << "\t" << chi2 << "\t" << ndf << endl; 
	    	    	    	    
	    if (printoption==0){
	     
	      canvas->SaveAs(Form("OFFSETCORR-CH%iCH%iDISC%iSUM%i.gif",channel,channely,disc,sum));
	    }
	    	    
	    if (printoption==1){
	      canvas->SaveAs(Form("COMBI-CH%iCH%iDISC%iSUM%i.pdf",channel,channely,disc,sum));
	      }
