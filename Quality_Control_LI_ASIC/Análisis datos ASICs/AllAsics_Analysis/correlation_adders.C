{
 ...
 // Read back the ntuple
  ...
  int asic, disc, channel, sum, channely, sumy;
...
for (disc = 0; disc < 2; disc++){
  ...
  for (sum = 0; sum < 3 ; sum++){
	...
	TString t= Form("Correlation Offset, Adder %c - %c Disc %i Channel %i",'A' + sum,'A' + sumy, disc,channel);
	...
	canvas->SaveAs(Form("ADDERSCORR-SUM%cSUM%cCH%iDISC%i.png",'A'+sum,'A'+sumy,channel,disc));
