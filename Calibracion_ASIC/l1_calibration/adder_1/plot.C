{
  ...
  for(int ip=0;ip<31;ip++){
    int n=t.Draw("value*4.8:pixels*70",Form("ip==%i && value>0",ips[ip]));
    if(n<3) continue;
  ...
  for (int asic=0; asic < 31; asic++){
   ...
   int n =  m->Draw("sumB*1000:input",Form("asic==%i && channel == 4 && disc == 0",asics[asic]));
   if(n<3) continue;
  ...
  TFile fout("ratescandata.root","RECREATE");
