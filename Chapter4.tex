...
\subsection{Analysis of Crab Campaigns with Expectation-Maximization method for Hillas Parameterization}

The data from the full three Crab campaigns was reduced from R0 to DL2 using the \gls{em} method to calculate the Hillas parameters, without using the tailcuts cleaning. To reconstruct the energy and direction of the primaries, and to perform the $\gamma$-hadron separation a new set of \glspl{rf} was trained were the \gls{mc} data was also parameterized using the \gls{em} method. The alpha plots for the three campaigns are shown in pictures \ref{fig:em_1st-crab-campaign}, \ref{fig:em_2nd-crab-campaign} and \ref{fig:em_3rd-crab-campaign} .\\
...
When using the \gls{em} method, the higher significance is reached with the lower cuts in intensity (intensity > 100), and a moderate gammaness cut (gammaness > 0.7), discarding less events than the baseline analysis from previous section, resulting in a higher $\gamma$ rate. However, it also raises the level of background events, as more events are classified with higher values of gammaness. The result show that the parameterization using the \gls{em} method allow to reconstruct and classify with higher gammaness showers with low number of photons, as was advanced in section \ref{sec:em}. This recover a large number of $\gamma$ events which do not survive the gammaness cut when using the tailcuts method, but on the other hand, keeps a large background of hadron events. These two effects compensate in the end resulting in a similar final significance as when using the tailcuts cleaning, with a lower cut in intensity and higher cut in gammaness.
